...
def OTU_profile(df):
    ...
    while s+windowsize != len(df.columns)+1:     # run an overlapping window
        ...
        p = 0
        for i in permut:
            frames = df.ix[(i[0],i[1]),s:s+10].transpose()
            corrMat.ix[i[0],i[1]] = frames.corr(method='pearson').ix[0,1]
            p+=1
            if o.verbose==True:
                ...
                sys.stdout.flush()
        x = []
        for i in frames.index:x.append(int(i.split('_')[0]))  # mean age of the window
        M = np.mean(x)
...
def OTU_profile_2(df):
    ...
    for i in permut:
        ...
        while s+windowsize != len(df.columns)+1:     # run a sliding window
            print s,s+windowsize
            frames = df.ix[(i[0],i[1]),53:53+windowsize].transpose()
            print frames
            print frames.corr(method='pearson').ix[0,1]
            sys.exit("stop")

            #x = []
            #for i in frames.index:x.append(int(i.split('_')[0]))  # mean age of the window
            #M = np.mean(x)
            #corr_array.append(frames.corr(method='pearson').ix[0,1])
            #window.append(M)
            s += 1
        sys.exit("stop")
        p+=1
